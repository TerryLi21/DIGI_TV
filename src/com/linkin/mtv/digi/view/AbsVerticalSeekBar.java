...
public class AbsVerticalSeekBar extends VerticalProgressBar {
	...
	public AbsVerticalSeekBar(Context context, AttributeSet attrs, int defStyle) {
		...
		mDisabledAlpha = a.getFloat(R.styleable.Theme_android_disabledAlpha,
				0.5f);
		a.recycle();
	}
	...
	public synchronized void setMax(int max) {
		...
		if ((mKeyProgressIncrement == 0)
				|| (getMax() / mKeyProgressIncrement > 20)) {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (thumbWidth > trackWidth) {
			...
			if (d != null) {
				...
				// start drawing
				d.setBounds(gapForCenteringTrack, 0, w - mPaddingRight
						- mPaddingLeft - gapForCenteringTrack, h
			...
			if (d != null) {
				// Canvas will be translated by the padding, so 0,0 is where we
				// start drawing
				d.setBounds(0, 0, w - mPaddingRight - mPaddingLeft, h
						- mPaddingBottom - mPaddingTop);
			}
